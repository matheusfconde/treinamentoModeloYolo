...
OUTPUT_DATASET_DIR = 'dataset/yolo_sem_seta'
...
def augment_icon(icon_img: Image.Image) -> Image.Image:
    # Mantém a imagem alinhada, sem rotação
...
def connect_icons(draw_obj: ImageDraw.Draw, bboxes: List[Tuple[int,int,int,int]], color: Tuple[int,int,int]):
    ...
    for i in range(len(shuffled_bboxes) - 1):
        ...
        # Função auxiliar: encontra ponto na borda mais próximo do outro centro
        def edge_point(bbox, dx, dy):
            ...
            if abs(dx) > abs(dy):
                ...
                if dy > 0:
                    ...
                    cy = y_min
                cx = cx + dx/dy * (cy - (y_min+y_max)/2)
            return (cx, cy)
        
        # Ajustar os pontos para ficarem na borda
        p1 = edge_point(bbox1, dx, dy)
        p2 = edge_point(bbox2, -dx, -dy)
        
        # Desenha linha
        draw_obj.line([p1, p2], fill=color, width=2)
        
        # Desenha seta na ponta
        angle = math.atan2(p2[1]-p1[1], p2[0]-p1[0])
        arrow_x1 = p2[0] - 10 * math.cos(angle - math.pi/6)
        arrow_y1 = p2[1] - 10 * math.sin(angle - math.pi/6)
        arrow_x2 = p2[0] - 10 * math.cos(angle + math.pi/6)
        arrow_y2 = p2[1] - 10 * math.sin(angle + math.pi/6)
        draw_obj.polygon([(p2[0], p2[1]), (arrow_x1, arrow_y1), (arrow_x2, arrow_y2)], fill=color)
